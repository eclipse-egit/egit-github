...
public class DownloadService extends GitHubService {
	 ...
	 */
	public DownloadService(
			org.eclipse.egit.github.core.client.HttpClient<?> client) {
		super(client);
	}
	...
	public void uploadResource(DownloadResource resource, InputStream content,
		...
		parts.put(UPLOAD_SUCCESS_ACTION_STATUS,
				Integer.toString(HttpStatus.SC_CREATED));
		...
		parts.put(UPLOAD_FILE, new SizedInputStreamPart(content, size));

		int status = client.postMultipart(resource.getS3Url(), parts);
