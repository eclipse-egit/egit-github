...
public class GitHubClient {
	...
	private <V> V sendJson(final HttpURLConnection request,
		...
		if (isOk(code))
			if (type != null)
				return parseJson(request.getInputStream(), type);
			else
				return null;
		if (isEmpty(code))
			return null;
		throw createException(request.getInputStream(), code,
	...
	private HttpURLConnection configure(final HttpURLConnection request) {
		if (credentials != null)
	...
	public InputStream getStream(GitHubRequest request) throws IOException {
		return createGet(request.generateUri()).getInputStream();
	}
	...
	public GitHubResponse get(GitHubRequest request) throws IOException {
		...
		throw createException(httpRequest.getInputStream(), code,
	...
	public void delete(final String uri, final Object params)
		...
		final int code = request.getResponseCode();
		if (!isEmpty(code))
			throw new RequestException(parseError(request.getInputStream()),
					code);
	}
}
