package org.eclipse.egit.github.pr.wizards;
...
import org.eclipse.egit.github.pr.Activator;
import org.eclipse.egit.github.pr.dialog.AuthenticationDialog;
...
public class PullRequestWizardPage extends WizardPage {
			...
			if (element instanceof RepositoryCommit) {
				...
				case 2:
					return new SimpleDateFormat("EEE, MMM dd, yyyy")
							.format(commit.getCommit().getAuthor().getDate());
				case 3:
	...
	public void createControl(Composite parent) {
		...
		grpPullRequestInfo.setLayoutData(new GridData(SWT.FILL, SWT.FILL, true,
				false, 4, 1));
		...
		toBranch.setLayoutData(new GridData(SWT.FILL, SWT.CENTER, true, false,
				1, 1));
		...
		fromBranch.setLayoutData(new GridData(SWT.FILL, SWT.CENTER, true,
				false, 1, 1));
		...
		try {
			...
			if (Activator.getDefault().getPreferenceStore()
				...
				String activeBranch = Activator.getDefault()
						.getPreferenceStore().getString(repositoryName);
		...
		btnGeneratePullRequest.addSelectionListener(new SelectionAdapter() {
			@Override
			public void widgetSelected(SelectionEvent e) {
				if (toBranch.getText().length() > 0
						&& fromBranch.getText().length() > 0
					...
					Activator.getDefault().getPreferenceStore()
							.setValue(repositoryName, toBranch.getText());
		...
		if (!preferences.nodeExists("eGitUserInfo")) {
			setMessage(
					"Use PreferenceStore to configure your GitHub Account to avoid being prompted for UserName and Password everytime.",
					...
					public void selectionChanged(SelectionChangedEvent event) {
						...
						if (selection instanceof IStructuredSelection) {
							...
							if (firstElement instanceof PullRequest) {
								...
								try {
									List<RepositoryCommit> commits = prService
											.getCommits(repo, pr.getNumber());
		...
		commitViewer = new TableViewer(tabFolder, SWT.BORDER
				| SWT.FULL_SELECTION);
		...
		try {
			...
			ContentProposalAdapter adapter = new ContentProposalAdapter(
					txtTitle, new TextContentAdapter(),
					new SimpleContentProposalProvider(commits
							.toArray(new String[commits.size()])),
	...
	private void makeActions() {
		copyUrlAction = new Action("Copy URL") {
			@Override
			public void run() {
				IStructuredSelection sSelection = ((IStructuredSelection) tableViewer
						.getSelection());
				if (!sSelection.isEmpty()) {
					PullRequest pullRequest = (PullRequest) sSelection
							.getFirstElement();
	...
	protected List<PullRequest> getPullRequests(boolean isCreation) {
		...
		for (PullRequest pullRequest : allPullRequests) {
			if (pullRequest.getHead().getRef().equals(toList[1])) {
				...
				if (pullRequest.getHead().getUser().getLogin()
						.equals(prService.getClient().getUser())) {
					pullRequestExists = true;
					errorMessage = pullRequest.getBase().getLabel() + " & "
		...
		if (!isCreation) {
			...
			if (pullRequestExists) {
				...
				setErrorMessage("Pull Request already Exists btw "
						+ errorMessage);
			} else {
	...
	protected void generatePullRequest() {
		...
		try {
			...
			PullRequest newPullRequest = prService.createPullRequest(repo,
					request);
			setErrorMessage(null);
	...
	private Set<String> getComments() {
		...
		String all = preferenceStore
				.getString(UIPreferences.COMMIT_DIALOG_HISTORY_MESSAGES);
		if (all.length() == 0)
			return Collections.emptySet();
		int max = preferenceStore
				.getInt(UIPreferences.COMMIT_DIALOG_HISTORY_SIZE);
		if (max < 1)
			return Collections.emptySet();
		XMLMemento memento;
		try {
